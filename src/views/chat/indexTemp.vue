...
export default {
  data() {
    return {
      //websocket部分
      // path: "ws://localhost:5501", //后台的websocket地址，找后端要
      // ws: null, //建立的连接
  ...
  mounted() {
    ...
    this.socket.on("chat message", this.reciveMsg);
    this.socket.on("connect", this.connect);
    this.socket.on("reconnecting", this.reconnecting);
    this.socket.on("disconnect", this.disconnect);
    this.socket.on("reconnect_refused", this.reconnectFailed);
    this.socket.on("error", this.catchError);
  },
  beforeDestroy() {
    console.log("beforeDestroy");
    // 取消连接
    this.socket.disconnect()
    // this.socket.off("chat message", this.reciveMsg);
    // this.socket.off("connect", this.connect);
    // this.socket.off("reconnecting", this.reconnecting);
    // this.socket.off("disconnect", this.disconnect);
    // this.socket.off("reconnect_refused", this.reconnectFailed);
    // this.socket.off("error", this.catchError);
    // 离开页面后关闭连接
    // this.ws.close();
    // // 清除时间
  ...
  methods: {
    ...
    //       self.ws.send(JSON.stringify({info:"ping"}));
    //     } else {
    ...
    //     //如果连接正常，给后天发送一个值，可以自定义，然后后台返回我们一个信息，我们接收到后会触发onmessage方法回调
    //     this.ws.send(JSON.stringify({info:'链接成功'})
    //     );
    //   }
    ...
    //   this.userInfoList2.push({info:e.info})
